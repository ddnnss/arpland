from django.contrib.auth.models import AbstractUser, BaseUserManager
from django.db import models
from ckeditor_uploader.fields import RichTextUploadingField
from pytils.translit import slugify
from random import choices

import string


class UserManager(BaseUserManager):
    """Define a model manager for User model with no username field."""

    use_in_migrations = True

    def _create_user(self, email, password, **extra_fields):
        """Create and save a User with the given email and password."""
        if not email:
            raise ValueError('The given email must be set')
        email = self.normalize_email(email)
        user = self.model(email=email, **extra_fields)
        user.set_password(password)
        user.save(using=self._db)
        return user

    def create_user(self, email, password=None, **extra_fields):
        """Create and save a regular User with the given email and password."""
        extra_fields.setdefault('is_staff', False)
        extra_fields.setdefault('is_superuser', False)
        return self._create_user(email, password, **extra_fields)

    def create_superuser(self, email, password, **extra_fields):
        """Create and save a SuperUser with the given email and password."""
        extra_fields.setdefault('is_staff', True)
        extra_fields.setdefault('is_superuser', True)

        if extra_fields.get('is_staff') is not True:
            raise ValueError('Superuser must have is_staff=True.')
        if extra_fields.get('is_superuser') is not True:
            raise ValueError('Superuser must have is_superuser=True.')

        return self._create_user(email, password, **extra_fields)


class User(AbstractUser):
    """User model."""

    username = None
    first_name = None
    last_name = None
    order_number = models.IntegerField('Порядок отображения', default=0)
    email = models.EmailField('Эл. почта', unique=True)
    name = models.CharField('ФИО', max_length=50, blank=True, null=True)
    phone = models.CharField('Телефон', max_length=50, blank=True, null=True)
    avatar = models.ImageField('Фото профиля', upload_to='avatar/', blank=True)
    
    profile_ok = models.BooleanField(default=False)


    USERNAME_FIELD = 'email'
    REQUIRED_FIELDS = []
    objects = UserManager()

    @property
    def get_avatar(self):
        if self.avatar:
            return self.avatar.url
        else:
            return 'data:image/png;base64,'


class Company(models.Model):
    user = models.ForeignKey(User, blank=False, null=True, on_delete=models.CASCADE,verbose_name='Принадлежит пользователю')
    name = models.CharField('Название организации', max_length=100, blank=False, null=True)
    name_slug = models.CharField(max_length=255, blank=True, null=True, editable=False)
    site = models.CharField('Сайт организации (без http)', max_length=50, blank=True, null=True)
    address = models.TextField('Адрес организации', blank=True, null=True)
    description = RichTextUploadingField('Полное описание организации', blank=True, null=True)
    short_description = models.CharField('Краткое описание организации', max_length=100, blank=True,
                                                      null=True)
    avatar = models.ImageField('Фото организации', upload_to='company/', blank=True)
    vk = models.CharField('Ссылка на VK', max_length=100, blank=True, null=True)
    fb = models.CharField('Ссылка на FB', max_length=100, blank=True, null=True)
    inst = models.CharField('Ссылка на Instagram', max_length=100, blank=True, null=True)
    yt = models.CharField('Ссылка на YouTube', max_length=100, blank=True, null=True)
    ok = models.CharField('Ссылка на OK', max_length=100, blank=True, null=True)
    views = models.IntegerField('Просмотров', default=0)
    created_at = models.DateTimeField(auto_now_add=True)

    @property
    def get_company_avatar(self):
        if self.avatar and hasattr(self.avatar, 'url'):
            return self.avatar.url
        else:
            return "http://placehold.it/900x500"
    def save(self, *args, **kwargs):
        slug = slugify(self.name)
        testSlug = Company.objects.filter(name_slug=slug)
        slugRandom = ''
        if not self.name_slug:
            slug = slugify(self.name)
            testSlug = Company.objects.filter(name_slug=slug)
            if testSlug:
                slugRandom = '-' + ''.join(choices(string.ascii_lowercase + string.digits, k=2))
                self.name_slug = slug + slugRandom
            else:
                self.name_slug = slug

        super(Company, self).save(*args, **kwargs)